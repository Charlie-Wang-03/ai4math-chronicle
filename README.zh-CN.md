...
Chronicle 维护一套**持续增长的 canonical 事件语料**，覆盖定理证明、形式化、数学发现、竞赛结果、数学专项系统、基准、数据集与基础设施。
...
本项目不是论文流，也不是模型排行榜。时间线只收录能够帮助解释 AI for Mathematics 历史演进的重要事件；随着新的里程碑出现，以及旧事件获得更强证据、纠错或新的历史背景，语料会持续演化。
...
重要性与 verification 状态严格分开：H1 事件仍然可能处于外部审查或活跃争议之中。
