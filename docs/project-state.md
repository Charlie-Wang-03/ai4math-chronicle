...
- **R1.2C-D remains stabilization, not expansion.** Final visual QA may tune or remove the already accepted R1.2C-B identity anchors and the retained footer echo across light/dark, English/Chinese, and desktop/mobile. It must not reintroduce the rejected chronology transition or Search & Explore zero-result ornament, nor create any new ornament surface, without another explicit owner decision.
